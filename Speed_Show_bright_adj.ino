...
#define BLUE 0xE6
             //|W|R|G|B|
#define WHITE 0xE6000000
#define PINK 0xDF04DB
#define ORANGE 0xFF1C00
#define CYAN 0xE4E4
#define PURPLE 0x8D00E4
#define YELLOW 0xE4E400

#define RED_HUE 0
#define GREEN_HUE 65536/3
#define BLUE_HUE (65536*2)/3
#define PINK_HUE (65536*5)/6
#define ORANGE_HUE (65536*23)/192
#define CYAN_HUE 65536/2
#define PURPLE_HUE 65536*.7611111
#define YELLOW_HUE 65536/6
...
// TIMSK0 = 0; // turn off timer0 for lower jitter
 ...
 // CODE ABOVE NEEDED FOR SOUND REACTIVE FUNCTION
  ...
  //Default values
  data = 's'; //choose the mode the device goes into initially
  color = ORANGE;
  hue = ORANGE_HUE;
}
...
// this fills in 11 LED's with interpolated values between each of the 8 OCT values 
       if (i>=2) {
        ...
        saturation=constrain(j+30, 200,255);
        saturation_prev=constrain(prev_oct_j+30, 200,255);
        brightness=constrain(j, 0,40);
        brightness_prev=constrain(prev_oct_j, 00,40);
        if (brightness==40) {
            saturation= 250;
            brightness=40;
        }
        if (brightness_prev==40) {
            saturation_prev=250;
            brightness_prev=40;
        }
  }
        ...
        for (uint8_t y=0;y<kMatrixHeight;y++){
            ...
            rgbcolor1 = strip.ColorHSV(hue,saturation, brightness);
            strip.setPixelColor(XY(led_index-1,y), rgbcolor1);      
          if (i>2){         
            prev_oct_j=(j+prev_j[i-1])/2;       
            rgbcolor2 = strip.ColorHSV(hue,saturation_prev, brightness_prev);
      ...
      if (prev_beat!=beat) {
        strip.setBrightness(10+beat*beat*5);
        prev_beat=beat;
      }
